...
Trialled and rejected for the hub (decision 17a, findings in `docs/personal-agent/EXEDEV-TRIAL.md`). Their VMs boot without an initrd and their base image compensates by hand; NixOS would need that work redone, and the platform offers no console to debug it against.
...
- [x] Scope exe.dev satellite (§8); rejected for the hub (decisions 17a, 18a; findings in `docs/personal-agent/EXEDEV-TRIAL.md`). Still open as a *satellite* for disposable sandboxes, where an Ubuntu base costs nothing.
